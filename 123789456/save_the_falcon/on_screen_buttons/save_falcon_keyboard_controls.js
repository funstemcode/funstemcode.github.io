...
animationScreenHeight = 300; //set the screen height
...
function render() {
    ...
    if (animationScreen.frameNo == 1 || everyinterval(150)) {
        x = animationScreen.canvas.width;
        minHeight = 20;
        maxHeight = 200;
        ...
        myObstacles.push(new animationObject(10, height, "green", x, 0));
        myObstacles.push(new animationObject(10, x - height - gap, "green", x, height + gap));
    }
    for (i = 0; i < myObstacles.length; i += 1) {
        myObstacles[i].x += -1;
