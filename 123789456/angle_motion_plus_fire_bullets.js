...
function render() {
    ...
    hero.moveAngle = 0;
    hero.speed = 0;
    if (animationScreen.keys && animationScreen.keys[37]) {hero.moveAngle = -(1**2); }
    if (animationScreen.keys && animationScreen.keys[39]) {hero.moveAngle = (1**2); }
    if (animationScreen.keys && animationScreen.keys[38]) {hero.speed+=(1**2); }
    if (animationScreen.keys && animationScreen.keys[40]) {hero.speed+=(-(1**2)); }
    if (animationScreen.keys && animationScreen.keys[32]) {
        bullet.push(new animationObject(1,1,"green",hero.x,hero.y));
        console.log(bullet);
        for(i=0; i<bullet.length; i++){

        bullet[i].newPosition();
        bullet[i].drawCharacter();
        bullet[i].x += 5;
        }
        }
